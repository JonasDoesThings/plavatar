...
func (server *Server) HandleGetMarbleAvatar() echo.HandlerFunc {
	return func(context echo.Context) error {
		...
		gradient := gg.NewRadialGradient(float64(size/2), float64(size/2), float64(size)/1.5, float64(size/2), float64(size/2), float64(size)/3)
		rng.Seed(seed)
